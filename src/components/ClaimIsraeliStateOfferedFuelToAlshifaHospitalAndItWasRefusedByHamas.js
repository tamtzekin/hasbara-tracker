...
export default function ClaimIsraeliStateOfferedFuelToAlshifaHospitalAndItWasRefusedByHamas() {    
    const metadataProps = {
        url: "https://hasbaratracker.com/israeli-state-offered-fuel-to-al-shifa-hospital-and-it-was-refused-by-hamas",
        title: summaries[2].claimMainTitle,
        description: summaries[2].claimSummary,
        
        twitterTitle: summaries[2].claimMainTitle,
        twitterDescription: summaries[2].claimSummary,
        twitterUrl: "https://hasbaratracker.com/israeli-state-offered-fuel-to-al-shifa-hospital-and-it-was-refused-by-hamas",
    };
    ...
    // All columns
    const columns = useMemo(() => TrackerColumns(), []);

    // filteredData based on selected claim title
    const filteredData = useMemo(() => {
        if (!selectedClaimTitle) {
            return data.filter(item => item.claimTitle === summaries[2].claimMainTitle);
        }
